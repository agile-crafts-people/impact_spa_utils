# Creators Dashboard — shared SPA utilities

Reusable Vue 3 + Vuetify components, composables, and utilities for **`impact_spa_utils`** and other Creators Dashboard SPAs.

This package follows the same pattern as `api_utils`, providing shared functionality across all SPA projects in the Creators Dashboard ecosystem.
...
{
  "dependencies": {
    "@agile-crafts-people/impact_spa_utils": "file:../impact_spa_utils"
  }
}
```
...
{
  "dependencies": {
    "@agile-crafts-people/impact_spa_utils": "github:agile-crafts-people/impact_spa_utils#main"
  }
}
```
...
After clone, npm runs the **`prepare`** script here, which builds **`dist/`** (this repo does not commit `dist`). **Private** `impact_spa_utils` needs git credentials (e.g. **`GITHUB_TOKEN`** in Docker, or `gh auth setup-git` locally).
...
- **Standards:** [SPA Standards](https://github.com/agile-crafts-people/impact/blob/main/DeveloperEdition/standards/spa_standards.md) - Creators Dashboard SPA development standards